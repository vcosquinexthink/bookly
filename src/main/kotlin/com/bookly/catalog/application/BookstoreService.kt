...
class BookstoreService {
    ...
    fun createBookstore(name: String, location: Location): Bookstore {
        val bookstore = Bookstore(UUID.randomUUID(), name, location, mutableMapOf())
        bookstores.add(bookstore)
        return bookstore
    }

    fun addBook(bookstoreId: UUID, book: Book, count: Int = 1): Boolean {
        val bookstore = bookstores.find { it.id == bookstoreId } ?: return false
    ...
    fun getInventoryForBook(bookstoreId: UUID, isbn: BookId): com.bookly.catalog.domain.model.InventoryItem? {
        val bookstore = bookstores.find { it.id == bookstoreId } ?: return null
    ...
    fun listInventory(bookstoreId: UUID): List<com.bookly.catalog.domain.model.InventoryItem> {
        val bookstore = bookstores.find { it.id == bookstoreId } ?: return emptyList()
